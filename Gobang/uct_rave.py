...
class uct_rave(object):
    ...
    def get_action(self,board,play_turn):
        ...
        while time.time()-begin<float(10) and simulations<5000:
    ...
    def simTree(self,board,play_turn,node,l,c_p):
        ...
        for _ in range(1, len(board.availables) + 1):
            ...
            #即经过模拟后
            if len(node)==l and len(availables)==0:
                #获取当前出手玩家
                player=board.get_player(play_turn)
                #遍历剩余节点
                #如果是当前玩家取胜率最大
                ...
                if c_p==player:
                    #ubc1公式
                    bonus,move=max(
                        (self.eval(node[key])+sqrt(log_total/node[key][0]),key) for key in ns)
    ...
    def update(self,node,n,sn,win,winer,c_p):
        ...
        for t in range(len(n)):
            if node.get(n[t]) is None:
                node[n[t]]=[0,0,0,0]
            node.get(n[t],[0,0,0,0])[0]+=1
            node.get(n[t],[0,0,0,0])[1]+=w
            if t%2==0:
                node.get(n[t],[0,0,0,0])[2]+=1
                node.get(n[t],[0,0,0,0])[3]+=w
